...
export default function UazapiTestPage() {
    ...
    const handleGetQrCode = async () => {
        ...
        try {
            const response = await fetch(`${SUPABASE_URL}/functions/v1/uazapi-integration?action=get_instance_status`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${session.access_token}`
                }
            ...
            // Extract QR code from instance status response
            if (data.success && data.data?.instance?.qrcode) {
                setQrCode(data.data.instance.qrcode)
                setResult(data)
            } else if (data.success && data.data?.connected) {
                setResult({ message: 'Instance already connected!' })
            } else {
                throw new Error('QR code not available. Instance may already be connected or disconnected.')
            }
        } catch (err: any) {
            setError(err.message)
        } finally {
